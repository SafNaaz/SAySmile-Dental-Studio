...
public class AnalyticsController {
    ...
    public Map<String, Object> getDashboardData(Authentication authentication) {
        ...
        if (user.getRole() == Role.PATIENT) {
            ...
            data.put("upcomingAppointments", appointmentService.getUpcomingAppointmentsByPatient(username, LocalDateTime.now(), LocalDateTime.now().plusMonths(1)));
            return data;
        }
        
        // Staff/Admin see everything
        data.put("upcomingAppointments", appointmentService.getUpcomingAppointments(LocalDateTime.now(), LocalDateTime.now().plusMonths(1)));
